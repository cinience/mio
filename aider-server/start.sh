...
DEFAULT_PORT="${DEFAULT_PORT:-8099}"
...
extract_port_from_config() {
  ...
  local addr_line
  addr_line=$(grep -E '^\s*listen_addr:\s*"' "$config_file" 2>/dev/null | head -n1 | sed -E 's/.*"([^"]+)".*/\1/' || true)
  if [[ -z "${addr_line:-}" ]]; then
    return 1
  fi
...
PORT="${AIDER_SERVER_PORT:-}"
if [[ -z "$PORT" ]] && ! PORT=$(extract_port_from_config "$CONFIG_PATH"); then
  PORT="$DEFAULT_PORT"
fi

echo "检查 aider-server 端口: $PORT"
check_and_kill_port "$PORT"

rm -rf logs/*
echo "Starting aider-server on port $PORT..."
