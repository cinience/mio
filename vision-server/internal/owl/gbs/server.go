...
import (
	...
	"vision-server/internal/owl/gbs/m"
	"vision-server/internal/owl/gbs/sip"
	"vision-server/internal/owl/ipc"
	"vision-server/internal/owl/netutil"
...
func (s *Server) API() *GB28181API {
	return s.gb
}

func NewServer(cfg *conf.Bootstrap, store ipc.Adapter, memoryStorer MemoryStorer, sc sms.Core) (*Server, func()) {
	api := NewGB28181API(cfg, store, sc.NodeManager)

	iip := netutil.InternalIP()
