...
func (s *UdpSession) Decrypt(data []byte) ([]byte, error) {
	...
	if data[0] != 0x01 {
		return nil, fmt.Errorf("invalid packet type: %d", data[0])
	}
	payloadLen := binary.BigEndian.Uint16(data[2:4])
	if int(payloadLen) != len(data)-16 {
		return nil, fmt.Errorf("payload length mismatch: declared=%d actual=%d", payloadLen, len(data)-16)
	}
	// 分离nonce和密文
	nonce := data[:16] // 使用16字节nonce
	ciphertext := data[16 : 16+payloadLen]
...
func (s *UdpSession) Encrypt(data []byte) ([]byte, error) {
	...
	copy(encrypted[4:8], s.Nonce[:4])                            // 4字节ssrc
	binary.BigEndian.PutUint32(encrypted[8:12], uint32(time.Now().UnixMilli()))
	s.LocalSeq++
