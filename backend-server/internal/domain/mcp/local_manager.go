...
// NewLocalMCPManager 构建一个新的本地管理器实例
func NewLocalMCPManager() *LocalMCPManager {
	manager := &LocalMCPManager{
		tools: make(map[string]*McpTool),
	}
	manager.initDefaultTools()
	return manager
}

// GetLocalMCPManager 获取默认MCP服务中的本地管理器
func GetLocalMCPManager() *LocalMCPManager {
	service := DefaultMCPService()
	if service == nil {
		return nil
	}
	return service.LocalManager()
}
...
func (l *LocalMCPManager) RegisterToolFunc(name, description string, inputParams any, handler LocalToolHandler) error {
	...
	if err != nil {
		log.Errorf("Failed to convert openapi3 schema to jsonschema: %v", err)
		return err
	}
	tool := &McpTool{
